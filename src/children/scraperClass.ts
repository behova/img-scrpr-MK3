import redditCore from './redditCore.js';
import fourChanCore from './fourChanCore.js';
...
class ScraperClass {
  ...
  private getSource(core: string): string {
    ...
    switch (core) {
      case 'redditCore':
        number = Math.floor(Math.random() * ScraperClass.redditSources.length);
        return ScraperClass.redditSources[number];
      case 'fourChanCore':
        number = Math.floor(
          Math.random() * ScraperClass.fourChanSources.length,
        );
        return ScraperClass.fourChanSources[number];
      default:
